...
import type { PlatformStatus, Post, PublishStatus } from "@/types";
...
function migratePost(
  post: Post &
    Partial<{ publishStatus: PublishStatus; channelConfig: unknown }>,
): Post {
  return {
    ...post,
...
// 根据环境选择存储方式
// 开发环境使用内存存储，生产环境优先使用KV存储（带降级）
let storage: StorageInterface;

// 强制使用内存存储的标志（用于当KV不可用时）
let useMemoryStorage = false;

if (process.env.NODE_ENV === "development") {
  storage = new MemoryStorage();
} else {
  // 生产环境：先尝试KV存储，如果失败则回退到内存存储
  storage = kvStorage;

  // 如果KV环境变量未配置，直接使用内存存储
  if (!process.env.KV_REST_API_URL) {
    console.warn("[Storage] KV not configured, using memory storage");
...
class PostStore {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      // 如果 storage 有 initialize 方法（如 KVStorage），先调用它
      ...
      console.error("[PostStore] Initialization failed:", error);
      // 如果初始化失败，切换到内存存储并重试
      if (!useMemoryStorage) {
        console.warn("[PostStore] Falling back to memory storage");
        useMemoryStorage = true;
        (storage as MemoryStorage) = new MemoryStorage();
  ...
  async createPost(
    postData: Omit<
      Post,
      "id" | "created_at" | "updated_at" | "publishStatus" | "channelConfig"
    >,
  ) {
    await this.initialize();
    const fullPostData = {
