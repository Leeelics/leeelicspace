...
function getDefaultLogLevel(): LogLevel {
  if (process.env.NODE_ENV === 'production') {
    ...
    switch (envLevel) {
      ...
      default: return LogLevel.WARN; // 生产环境默认只显示警告和错误
    }
  }
...
class Logger {
  ...
  private output(level: LogLevel, message: string, context?: Record<string, unknown>, error?: Error): void {
    ...
    const styledPrefix = styles[level] ? styles[level].replace('%s', prefix) : prefix;
  ...
  // API 路由专用日志方法
  api(request: Request, message: string, context?: Record<string, unknown>): void {
  ...
  apiError(request: Request, message: string, error: Error, context?: Record<string, unknown>): void {
    const url = new URL(request.url);
