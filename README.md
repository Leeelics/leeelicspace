...
1. 访问 `/dashboard/login` 页面。
2. 输入管理员密钥（默认为 `admin-secret`）。
3. 登录成功后，系统会将登录状态存储在浏览器 `localStorage`，并自动跳转到后台首页。

### 4.4.2 进入文章管理页面

1. 登录后台后，访问 `/dashboard/posts` 页面。
2. 可查看所有文章列表，支持编辑和删除操作。

### 4.4.3 创建新文章

1. 在后台首页点击“新建文章”或直接访问 `/dashboard/create` 页面。
2. 填写文章标题、内容、标签等信息。
3. 提交时会自动携带管理员密钥（无需手动输入）。
4. 创建成功后会跳转到文章列表或详情页。

### 4.4.4 权限说明

- 所有后台管理操作均需登录，未登录会自动跳转到登录页。
- 文章创建、编辑、删除等接口均需携带 `secret` 参数，前端已自动处理。
