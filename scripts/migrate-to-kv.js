...
async function migrateFromFileToKV() {
  ...
  try {
    ...
    try {
      ...
      console.log('ℹ️  No existing file storage found or error reading file:', error.message);
    ...
    if (existingData.length === 0 && currentKVData.length === 0) {
      ...
      console.log('   The application will create initial data on first API call.');
      return;
    }

    if (existingData.length > 0 && currentKVData.length === 0) {
      console.log('✅ Migration needed: File storage has data, KV storage is empty');
      ...
      const validPosts = existingData.filter(post => {
        const required = ['id', 'title', 'content', 'tags', 'created_at', 'updated_at'];
        const hasAllRequired = required.every(field => post.hasOwnProperty(field));
      ...
      if (validPosts.length > 0) {
        ...
        console.log(`✅ Successfully migrated ${validPosts.length} posts to KV storage`);
      ...
      const confirm = await askQuestion('Do you want to overwrite KV storage with file storage data? (y/N): ');
