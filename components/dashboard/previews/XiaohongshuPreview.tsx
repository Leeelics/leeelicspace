...
export default function XiaohongshuPreview({ title, content }: XiaohongshuPreviewProps) {
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="text-sm">
          ...
          <span className={isOverLimit ? 'text-red-500' : 'text-[var(--text-primary)]'}>
        ...
        >
          ...
          <div style={{ color: config.textColor }}>
            ...
            <div style={{ fontSize: config.fontSize, lineHeight: 1.6 }}>
              <ReactMarkdown 
                ...
                components={{
                  h1: ({ children }) => <div style={{ fontSize: config.fontSize * 1.3, fontWeight: 'bold', margin: '0.5em 0' }}>{children}</div>,
                  h2: ({ children }) => <div style={{ fontSize: config.fontSize * 1.2, fontWeight: 'bold', margin: '0.5em 0' }}>{children}</div>,
                  p: ({ children }) => <div style={{ margin: '0.5em 0' }}>{children}</div>,
                  li: ({ children }) => <div style={{ margin: '0.3em 0', paddingLeft: '1em' }}>• {children}</div>,
                }}
              >
          ...
          {config.showWatermark && (
            ...
            >
              ...
              <span className="ml-auto">{new Date().toLocaleDateString('zh-CN')}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-6 p-4 bg-[var(--surface)] rounded-lg">
        <h3 className="text-sm font-medium text-[var(--text-primary)] mb-3">样式设置</h3>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">背景</label>
            <select
            ...
            >
              <option value="linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)">极光</option>
              <option value="linear-gradient(120deg, #f6d365 0%, #fda085 100%)">日落</option>
          ...
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">字号</label>
            <input
              type="range"
              min={20}
              max={40}
